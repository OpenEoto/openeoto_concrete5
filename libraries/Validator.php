...
// MA -> removed DNS check in email method
// MA -> added linebreak-removal
 ...
 */
/* BY MA */
function remove_linebreaks($val) {
    $val = str_replace(array("\r\n", "\n", "\r"), ' ', $val); // 2DO: double coded? - is this needed?
    return $val;
}
...
class Validator {
    ...
    public function integer($message = null) {
        $this->setRule(__FUNCTION__, function($val) {
                    // previous version did not cover 0 as valid zero
                    // 2DO: pull request?
                    // http://www.cleancode.co.nz/blog/664/filter_validate_int-0
                    // special case for '00' - '09'
                    if (is_string($val)) {
                        if ($val === '00' || $val === '0') {
                            $val = 0;
                        } else {
                    ...
                    return (filter_var($val, FILTER_VALIDATE_INT) === 0 || !filter_var($val, FILTER_VALIDATE_INT) === FALSE);
    ...
    public function maxlength($len, $message = null) {
        $this->setRule(__FUNCTION__, function($val, $args) {
                    return !(mb_strlen(trim($val)) > $args[0]); // BY MA - use mb_strlen
                }, $message, array($len));
        return $this;
    }
    ...
    public function length($len, $message = null) {
        $this->setRule(__FUNCTION__, function($val, $args) {
                    return (mb_strlen(trim($val)) == $args[0]);
                }, $message, array($len));
        return $this;
    }
    ...
    public function url($message = null) {
        $this->setRule(__FUNCTION__, function($val) {
                    // BY MA - added FILTER_FLAG_SCHEME_REQUIRED
